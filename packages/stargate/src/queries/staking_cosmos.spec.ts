...
import { setupCosmosStakingExtension, CosmosStakingExtension } from "./staking_cosmos";

async function makeClientWithStaking(
  rpcUrl: string,
): Promise<[QueryClient & CosmosStakingExtension, Tendermint34Client]> {
  const tmClient = await Tendermint34Client.connect(rpcUrl);
  return [QueryClient.withExtensions(tmClient, setupCosmosStakingExtension), tmClient];
}

describe("CosmosStakingExtension", () => {
  const defaultFee = {
